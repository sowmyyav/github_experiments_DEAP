# -*- coding: utf-8 -*-
"""
Created on Mon Oct  4 14:35:25 2021
...
#this was run with threshold 5 and added one conv1d layer ran for 1000 epochs
#added dropout at all layers and increase to 0.4 from 0.3

model2a = Sequential()
model2a.add(Conv1D(filters=64, kernel_size=3,  strides=1, activation='relu', input_shape=(x_emg_train.shape[1],x_emg_train.shape[2])))
...
model2a.add(BatchNormalization())
model2a.add(Dropout(0.4))
model2a.add(Bidirectional(LSTM(64)))
model2a.add(BatchNormalization())
model2a.add(Dropout(0.4))
...
mc = ModelCheckpoint('emg2channels_500bs_1000epochs_128w_32o_model2a.h5', monitor='accuracy', mode='max', verbose=1, save_best_only=True)
model2a.summary()
# fit network
history=model2a.fit(x_emg_train, y_emg_train_resampled, epochs=1000, batch_size=500, verbose=1, callbacks = [es,mc],validation_data=(x_emg_vald, y_emg_vald_resampled))
