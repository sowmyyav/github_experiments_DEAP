...
X_train_emg_val, X_test_emg_val, y_train_emg_val, y_test_emg_val = train_test_split(emg_data, y_valence, test_size=0.33, random_state=42, stratify=y_valence)
...
#this was run with threshold 5 and train test split 0.33, dropout =  0.3 all layers

model4 = Sequential()
model4.add(Conv1D(filters=32, kernel_size=3,  strides=1, activation='relu', input_shape=(x_emg_train.shape[1],x_emg_train.shape[2])))
model4.add(BatchNormalization())  
model4.add(Dropout(0.3))
model4.add(Conv1D(filters=64, kernel_size=3,  strides=1, activation='relu', input_shape=(x_emg_train.shape[1],x_emg_train.shape[2])))
model4.add(BatchNormalization())  
model4.add(Dropout(0.3))
model4.add(Conv1D(filters=64, kernel_size=3,  strides=1, activation='relu'))
model4.add(BatchNormalization())  
model4.add(Dropout(0.3))
...
mc = ModelCheckpoint('emg2channels_1000bs_128w_32o_model4_git.h5', monitor='accuracy', mode='max', verbose=1, save_best_only=True)
model4.summary()
# fit network
history4=model4.fit(x_emg_train, y_emg_train_resampled, epochs=1000, batch_size=500, verbose=1, callbacks = [es,mc],validation_data=(x_emg_vald, y_emg_vald_resampled))
