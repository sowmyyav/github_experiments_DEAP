...
model3 = Sequential()
model3.add(Conv1D(filters=32, kernel_size=3,  strides=1, activation='relu', input_shape=(x_emg_train.shape[1],x_emg_train.shape[2])))
model3.add(BatchNormalization())  
model3.add(Dropout(0.3))
model3.add(Conv1D(filters=64, kernel_size=3,  strides=1, activation='relu', input_shape=(x_emg_train.shape[1],x_emg_train.shape[2])))
model3.add(BatchNormalization())  
...
mc = ModelCheckpoint('emg2channels_1000bs_128w_32o_model3_git.h5', monitor='accuracy', mode='max', verbose=1, save_best_only=True)
model3.summary()
# fit network
history=model3.fit(x_emg_train, y_emg_train_resampled, epochs=1500, batch_size=500, verbose=1, callbacks = [es,mc],validation_data=(x_emg_vald, y_emg_vald_resampled))
