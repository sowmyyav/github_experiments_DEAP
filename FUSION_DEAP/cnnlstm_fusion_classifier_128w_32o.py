...
print(X_train_ff_val.shape)
print(X_test_ff_val.shape)


#use stratify for split   
X_test1_ff_val, X_vald_ff_val, y_test1_ff_val, y_vald_ff_val = train_test_split(X_test_ff_val, y_test_ff_val, test_size=0.5, random_state=42, stratify=y_test_ff_val)
print(X_vald_ff_val.shape)
print(X_test1_ff_val.shape)
...
X_ff_test1_resampled, y_ff_test1_resampled = ros.fit_resample(X_test1_ff_val, y_test1_ff_val)
print(X_ff_train_resampled.shape)
print(X_ff_vald_resampled.shape)
print(X_ff_test1_resampled.shape)
...
print(counter) 
counter = Counter(y_ff_test1_resampled)
print(counter) 
...
#y_test
Z22 = np.ravel(y_ff_test1_resampled)
y_ff_test1_resampled = to_categorical(Z22)
...
ff_testing1_set_scaled = sc.transform(X_ff_test1_resampled)
...
x_ff_test1 = ff_testing1_set_scaled.reshape(ff_testing1_set_scaled.shape[0], 128, n_features, order='F')
print(x_ff_train.shape)
print(x_ff_vald.shape)
print(x_ff_test1.shape)
...
mc = ModelCheckpoint('ff8channels_1000bs_128w_32o_model2_git.h5', monitor='accuracy', mode='max', verbose=1, save_best_only=True)
model2.summary()
# fit network
history=model2.fit(x_ff_train, y_ff_train_resampled, epochs=1000, batch_size=500, verbose=1, callbacks = [es,mc],validation_data=(x_ff_vald, y_ff_vald_resampled))
