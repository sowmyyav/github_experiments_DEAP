...
with strategy.scope():
    ...
    model1 = Sequential()
    model1.add(Conv1D(filters=64, kernel_size=3,  strides=1, activation='relu', input_shape=(x_ff_train.shape[1],x_ff_train.shape[2])))
    model1.add(BatchNormalization())  
    #model1.add(Dropout(0.3))
    model1.add(Conv1D(filters=64, kernel_size=3,  strides=1, activation='relu'))
    model1.add(BatchNormalization())  
    #model1.add(Dropout(0.3))
...
es = EarlyStopping(monitor='val accuracy', mode='max', verbose=1, patience=50)
