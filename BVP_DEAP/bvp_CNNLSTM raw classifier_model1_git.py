...
X_train_bvp_val, X_test_bvp_val, y_train_bvp_val, y_test_bvp_val = train_test_split(bvp_data1, y_valence, test_size=0.2, random_state=42, stratify=y_valence)
...
with strategy.scope():
    model1 = Sequential()
    model1.add(Conv1D(filters=64, kernel_size=3,  strides=1, padding="causal", activation='relu', input_shape=(x_bvp_train.shape[1],x_bvp_train.shape[2])))
...
mc = ModelCheckpoint('BVP_256bs_128w_32o_model1.h5', monitor='accuracy', mode='max', verbose=1, save_best_only=True)
model1.summary()
# fit network
history=model1.fit(x_bvp_train, y_bvp_train_resampled, epochs=200, batch_size=256, verbose=1,validation_data=(x_bvp_test, y_bvp_test_resampled))
