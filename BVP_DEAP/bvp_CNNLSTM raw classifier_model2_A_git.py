...
with strategy.scope():
    #added dropout after conv1d and increase epochs
    model2a = Sequential()
    #model2.add(Conv1D(filters=32, kernel_size=3,  strides=1, padding="causal", activation='relu', input_shape=(x_bvp_train.shape[1],x_bvp_train.shape[2])))
    model2a.add(Conv1D(filters=64, kernel_size=3,  strides=1, padding="causal", activation='relu', input_shape=(x_bvp_train.shape[1],x_bvp_train.shape[2])))
    model2a.add(Conv1D(filters=128, kernel_size=3, activation='relu'))
    model2a.add(Dropout(0.3))
    model2a.add(Bidirectional(LSTM(64, return_sequences=True)))
    model2a.add(Dropout(0.3))
    model2a.add(Bidirectional(LSTM(128)))
    model2a.add(Dropout(0.3))
    #model.add(MaxPooling1D(pool_size=2))
    #model.add(Flatten())
    model2a.add(Dense(100, activation='relu'))
    model2a.add(Dropout(0.3))
    model2a.add(Dense(2, activation='softmax'))
...
mc = ModelCheckpoint('BVP_256bs_128w_32o_model2_a.h5', monitor='accuracy', mode='max', verbose=1, save_best_only=True)
model2a.summary()
# fit network
history=model2a.fit(x_bvp_train, y_bvp_train_resampled, epochs=1000, batch_size=256,callbacks = [es,mc], verbose=1,validation_data=(x_bvp_test, y_bvp_test_resampled))
